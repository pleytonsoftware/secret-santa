  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        {!isDisabled && (
          <div className="absolute inset-0 bg-gradient-to-r from-primary via-accent to-secondary rounded-xl blur-lg opacity-30 animate-pulse" />
        )}
        ...
        <button
          ...
          className={`relative btn btn-lg w-full gap-3 text-lg ${
            isDisabled
              ? "btn-disabled"
              : "btn-primary bg-gradient-to-r from-primary via-primary to-secondary border-0 hover:from-primary/90 hover:to-secondary/90 shadow-xl hover:shadow-2xl transform hover:scale-[1.02] transition-all duration-300"
          }`}
        >
          {isLoading ? (
            <>
              <span className="loading loading-spinner loading-md"></span>
              <span className="animate-pulse">{t("randomize.sending")}</span>
      ...
      {!isFinalized && participantCount >= 2 && (
        <div className="flex items-center justify-center gap-2 text-sm text-base-content/50">
          <span>💡</span>
          <span>{t("randomize.tip") || "This action is permanent and will send emails to all participants."}</span>
        </div>
      )}
    </div>
  );
